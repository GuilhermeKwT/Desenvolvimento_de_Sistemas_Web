...
export default class TicketsController {
    ...
    public async create(request: Request, response: Response, next: NextFunction): Promise<Response | void>{
        try {
            const {film, seats, session_date, room, clientId} = request.body;

            const createTicket = new CreateTicketService();
            const Ticket = await createTicket.execute({film, seats, session_date, room, clientId});
    ...
    public async update(request: Request, response: Response, next: NextFunction): Promise<Response | void>{
        try {
            ...
            const Ticket = await updateTicket.execute({id, film, seats, session_date, room, clientId});
